...
static subbus_cache_word_t ps_spi_cache[PS_SPI_HIGH_ADDR - PS_SPI_BASE_ADDR+1] = {
	{ 0, 0, true,  false,  false,  false, false },	// W,   Raw,   Temperature,	Pressure Sensor 1
	{ 0, 0, true,  false,  false,  false, false },	// LSW, Raw,   Pressure,	Pressure Sensor 1
	{ 0, 0, true,  false,  false,  false, false },	// MSW, Raw,   Pressure,	Pressure Sensor 1
	...
	{ 0, 0, true,  false,  false,  false, false },	// W,   Raw,   Temperature,	Pressure Sensor 2
	{ 0, 0, true,  false,  false,  false, false },	// LSW, Raw,   Pressure,	Pressure Sensor 2
	{ 0, 0, true,  false,  false,  false, false },	// MSW, Raw,   Pressure,	Pressure Sensor 2
...
void ps_cache_update(EEPROM_t *sensor, uint8_t which_prom){
	uint8_t indx = ((which_prom-1)*PS_NUM_WORDS_RAW)+PS_SPI_BASE_ADDR;
	subbus_cache_update(&sb_ps_spi, 0+indx, (uint16_t) prom_p->temperature_raw);
	subbus_cache_update(&sb_ps_spi, 1+indx, (uint16_t) prom_p->pressure_raw >> 16);
	subbus_cache_update(&sb_ps_spi, 2+indx, (uint16_t) prom_p->pressure_raw);
  ...
  //ps_spi_cache[2+indx].cache = (uint16_t) prom_p->pressure_raw;
	...
	subbus_cache_update(&sb_ps_spi, 3+indx, (uint16_t) f2b.w[0]);
	subbus_cache_update(&sb_ps_spi, 4+indx, (uint16_t) f2b.w[1]);
  //ps_spi_cache[3+indx].cache = f2b.w[0];
  //ps_spi_cache[4+indx].cache = f2b.w[1];
	
	f2b.f = prom_p->pressure;
	subbus_cache_update(&sb_ps_spi, 5+indx, (uint16_t) f2b.w[0]);
	subbus_cache_update(&sb_ps_spi, 6+indx, (uint16_t) f2b.w[1]);
